...
/**
  ...
  */
object rechargeStreaming {
  def main(args: Array[String]): Unit = {
    //主题和消费组
    val group="sh001"
    val topic="bike_recharge"
    ...
    stream.foreachRDD { rdd =>
      ...
      val baseData = rdd.map(t => JSON.parseObject(t.value()))
        .map(jsonObj => {
          ...
          val rechargesource=jsonObj.getString("rechargesource")
          ("recharge"+date,province,city,client,deviceType,hour,rechargetype,amount,rechargesource)
        })
      ...
      /**
        * 支付源头（1支付宝 2微信 3银行卡）数量及现金流量
        */
      baseData.map( t =>(t._1+"+rechargesource+"+t._9,List[Double](t._8)))
        .reduceByKey((a,b) =>{
          ( a zip b) map (x => x._1+x._2)
        }).foreachPartition(itr =>{
        val client = JedisUtils.getJedisClient
